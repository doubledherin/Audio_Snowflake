...
API_URL = "http://developer.echonest.com/api/v4/song/search"


def get_matching_songs(artist=None, title=None):
    """
    Returns a list of songs that match the params.
    Note: This function isn't called if no artist or title are passed in.
    """
    ...
    try:
        ...
        return "Sorry, that song is unavailable. Please try a different one."
...
def get_song_data(songs):
    ...
    for song in songs:
        ...
        # Filter for a track that has a Spotify track URI
        else:
            for track in tracks:
...
def collapse_sections(section_results):
    ...
    for i in range(len(section_results)):
        ...
        collapsed[(key, mode, time_sig)] = \
            collapsed.setdefault((key, mode, time_sig), [])
    ...
    for key, value in items:
        ...
        for item in value:
            ...
            total_time_sig_confidence += item["time_signature_confidence"]
        ...
        totals = [
            ...
            total_mode_confidence,
            total_time_sig_confidence,
            total_tempo,
            total_loudness
        ]
...
def algorithm(artist=None, title=None):
    """
    Takes an artist name and song title and
    returns a complete dictionary of song data
    """
    ...
    # This list will be passed is used create the legend
    sections = []
...
def scaler(x, a, b, c, d):
    """
    Takes a number x within range [a, b] and scales
    it according to the desired range [c, d]
    """
...
def hsv2hsl(hue, sat, val):
    """
    Takes 3 numbers representing an HSV color and returns
    a list of 3 numbers representing the equivalent color as HSL
    """
